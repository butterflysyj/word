...
const speak = (text: string, lang = 'en-US') => {
    if ('speechSynthesis' in window) {
        // 모바일 최적화 매니저 인스턴스 가져오기
        const mobileManager = MobileOptimizationManager.getInstance();
        
        // 모바일에서 오디오 컨텍스트가 활성화되지 않은 경우 강제 활성화
        if (mobileManager.isMobileDevice() && !mobileManager.isAudioContextUnlocked()) {
        ...
        setTimeout(() => {
            ...
            // 모바일 최적화된 음성 설정
            utterance.rate = 0.7; // 모바일에서 더 느리게
            utterance.pitch = 1.0;
            ...
            const findAndSpeak = () => {
                ...
                // 1. 모바일 선호 음성 찾기
                for (const voiceName of mobilePreferredVoices) {
                    bestVoice = voices.find(voice => 
                        voice.lang.startsWith('en') && 
                        (voice.name.includes(voiceName) || voice.name === voiceName)
                    );
                    if (bestVoice) {
                        console.log(`Found preferred mobile voice: ${bestVoice.name}`);
                        break;
                    }
                }
                
                // 2. 영어 음성 중에서 로컬 음성 우선 선택
                if (!bestVoice) {
                    bestVoice = voices.find(voice => 
                        voice.lang.startsWith('en') && voice.localService === true
                    );
                    if (bestVoice) {
                        console.log(`Found local English voice: ${bestVoice.name}`);
                    }
                }
                ...
                if (!bestVoice && voices.length > 0) {
                    bestVoice = voices[0];
                    console.log(`Using default voice: ${bestVoice.name}`);
                }
            ...
            if (speechSynthesis.getVoices().length > 0) {
                ...
                const waitForVoices = () => {
                    voiceLoadAttempts++;
                    if (speechSynthesis.getVoices().length > 0 || voiceLoadAttempts >= maxAttempts) {
        ...
        <div className="p-4 sm:p-8 flex flex-col items-center">
            ...
            <div className="w-full max-w-lg bg-slate-700 rounded-xl shadow-2xl p-6 sm:p-8 text-center">
                <div className="mb-2">
                    ...
                    }} className="text-slate-400 hover:text-cyan-400 text-2xl" aria-label="단어 발음 듣기">
                        🔊
                    </button>
                </div>
                            ...
                            <div className="flex justify-between items-start">
                                ...
                                <div className="flex flex-col sm:flex-row space-y-1 sm:space-y-0 sm:space-x-1 flex-shrink-0 ml-2 items-end">
                                     <button onClick={() => {
                                        ...
                                        mobileManager.triggerHapticFeedback('light');
                                        speak(word.term);
                                    }} className="text-slate-400 hover:text-cyan-400 text-xl p-1.5 rounded-md hover:bg-slate-500" aria-label={`${word.term} 발음 듣기`}>
                                        🔊
                                    </button>
                                    <button 
